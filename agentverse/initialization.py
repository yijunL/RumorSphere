...
def load_abm_model(abm_config: Dict) -> mesa.Model:
    ...
    profile_path = abm_config.pop("agent_config_lst",None)
    opinion_shift = abm_config.pop("opinion_shift",0)
    if(profile_path==None):
        raise ValueError(
            "abm model.agent_config_lst does not exist, it should be aligned with agents.profile_path!"
        )
    ...
    if os.path.exists(profile_path):
        ...
        print(f"{path} does not exist, assigning default values to all agents")
    ...
    for i in range(len(profiles["id"])):
        abm_profiles.append({
            ...
            "init_att": profiles["init_att"][i]+opinion_shift
        })
        
    abm_config["agent_config_lst"] = abm_profiles
...
def prepare_task_config(task, tasks_dir):
    ...
    # read from file------------------------------------
    profiles={}
    profile_path = task_config["agents"][0].get("profile_path")
    if os.path.exists(profile_path):
        df = pd.read_csv(profile_path)
        profiles = df.to_dict(orient='list')
    else:
    ...
    new_task_config = []
    sample_agent_config = task_config["agents"][0]
    agent_num = sample_agent_config.get("agent_num", 10)
    for i in range(agent_num):
        tmp = copy.deepcopy(sample_agent_config)
        tmp["name"] = profiles["name"][i]
        tmp["role_description"] = profiles["role_description"][i]
        new_task_config.append(tmp)
    task_config["agents"] = new_task_config
    # read from file------------------------------------
