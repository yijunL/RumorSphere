...
class TwitterEnvironment(BaseEnvironment):
    ...
    recoder = {"atts":[], "core_agent_ids":[],"messages":[],"spend":[]}
    class Config:
    ...
    async def step(self) -> List[Message]:
        ...
        self.recoder["atts"].append(self.abm_model.get_attitudes())
        self.recoder["core_agent_ids"].append(agent_ids)
        self.recoder["spend"].append(sum([agent.get_spend() for agent in self.agents]))
        if(len(self.recoder["messages"])==0):
            self.recoder["messages"].append("")

        df = pd.DataFrame({
            'atts': self.recoder["atts"],
            'coreagent': self.recoder["core_agent_ids"],
            'messages': self.recoder["messages"],
            'total_spend': self.recoder["spend"]
        })
        
        # 将DataFrame保存到CSV文件
        df.to_csv(f'output/opinions_num_{len(self.agents)}_alpha_{self.abm_model.alpha}_bound_{self.abm_model.bc_bound}_core_{self.core_agent_num}_epoch_{self.max_turns}_{self.target}.csv', index=False, encoding='utf-8')

        # print("Get the personal experience of each agent...")
        ...
        # print("Get the personal experience of each agent completed.")

        # Generate current environment description
        
        # print("Generate current environment description...")
        env_descriptions = self.rule.get_env_description(self)
        # print("Generate current environment description completed.")
        ...
        self.recoder["messages"].append(messages)

        print("***get agent message completed...***")
        ...
        if self.abm_model is not None:
            ...
            #     self.abm_model.update_mirror(self.agents[i].name, self.agents[i].atts[-1])
                ...
                #     self.abm_model.update_mirror(self.agents[i].name, -1)
        print("***abm_model.step() completed***")
