...
limiter = AsyncLimiter(3, 1) #控制并发数量
...
class TwitterAgent(BaseAgent):
    ...
    def att2score(self, content): # use Textblob
        # label stance
        score = label_stance(content, self.environment.target)
        # if stance in ['Rejection']:
        #     sign=-1
        # else:
        #     sign=1
    ...
    def step(self, current_time: dt, env_description: str = "") -> Message:
        ...
        if len(self.memory.messages):
            ...
            self.retrieved_memory = self.memory_manipulator.retrieve(questions, 5)
        else:
        ...
        if reaction is None:
            reaction = "Silence"
            reaction_type = 'other'
            att = self.atts[-1] if len(self.atts)!=0 else 0
            stance = 'Doubt'
            if(att<0):
                stance = "Rejection"
            elif(att>0):
    ...
    async def astep(self, current_time: dt, env_description: str = "") -> Message:
        ...
        async with limiter:
            ...
            # print("self.max_retry:",self.max_retry)
    ...
    async def acontext_test(self, context_config) -> Message:
        ...
        if reaction is None:
            reaction = "Silence"
            reaction_type = 'other'
            att = self.atts[-1] if len(self.atts)!=0 else 0 if len(self.atts)!=0 else 0
            stance = 'Doubt'
            if(att<-0.2):
                stance = "Rejection"
            elif(att>0.2):
